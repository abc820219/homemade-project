// // SELECT * FROM teacher_like tl inner join teacher t on tl.teacher_sid = t.teacher_sid where member_sid = 80
...
const removeLikeCourse = (req, res) => {
    const { memberId } = req.session
    const { courseId } = req.body
    const sql = "update course_like set status = 0 where member_sid = ? and course_sid = ?"
    const placeholders = [memberId, courseId]
    const callback = (err, data) => {
        if (err) {
            res.send({
